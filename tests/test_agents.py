...
class TestTextQATaskDataset:
    ...
    async def test_figqa_retrieval(self, tmp_path, reader: str) -> None:
        ...
        def make_individual_env(idx: int) -> GradablePaperQAEnvironment:
            env = dataset.get_new_env_by_idx(idx)
            (doi,) = env.sources
            if doi == "10.1016/j.cub.2025.06.060":
                doi = "10.1101/2025.02.28.640764"
            elif doi == "10.1016/j.cub.2025.08.023":
                doi = "10.1101/2024.09.11.611976"
            elif doi == "10.1016/j.jbc.2024.106794":
                doi = "10.1101/2023.07.10.548331"
            try:
                (details,) = (
                    v for v in manifest.values() if v.get("doi") == doi.lower()
                )
